    ...
    def start(self, timeout=5):
        ...
        while end_time - start_time < timeout:
            ...
            if json_str is not None:
                print(f"Found img info for {self.camera_name}, attempting to start...")
                for _ in range(5):
                    self.save_img(flag=True)
                    time.sleep(0.02)

                img_info = self.get_img_info()
                print(f"Retrieved image info: {img_info}")  # Debug output
                self.camera_type = img_info["camera_type"]
                return True
            end_time = time.time()

        raise ValueError(f"Failed to get image info for {self.camera_name} within timeout")
