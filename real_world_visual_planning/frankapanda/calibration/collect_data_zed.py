...
from hacman_real_env.pcd_obs_env.zed_cams import ZedCams
...
def move_robot_and_record_data(
    ...
    cam = ZedCams(cam_ids=[cam_id])
    camera_matrix = cam.cam_params.camera_matrix1
    dist_coeffs = cam.cam_params.dist_coeffs
    ...
    for _ in tqdm(range(num_movements)):
        ...
        frame = cam.get_image(cam_id)
        frame = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
        if frame is not None:
            # Detect ArUco markers and get visualization
            corners, ids = detect_aruco_markers(frame, debug=debug)
    ...
    # Save data
    os.makedirs(f"hacman_real_env/pcd_obs_env/zed_cams/data", exist_ok=True)
    filepath = f"hacman_real_env/pcd_obs_env/zed_cams/data/cam{cam_id}_data.pkl"
    with open(f"hacman_real_env/pcd_obs_env/zed_cams/data/cam{cam_id}_data.pkl", "wb") as f:
        pickle.dump(data, f)
    return filepath
